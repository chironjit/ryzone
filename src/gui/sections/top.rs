...
pub fn view(state: &State) -> Element<Message> {
    column![
        // Top bar
        row![
            container(text("Ryzone - Control Mobile Ryzen Power States"))
...
fn create_processor_view(state: &State) -> Element<Message> {
    column![
        row![
            column![text("CPU").size(20), text("Current").size(10),].spacing(2),
            Space::with_width(Length::Fill),
            column![text(format_frequency(state.current_max_freq)).size(28),]
        ]
...
fn create_gpu_view(state: &State) -> Element<Message> {
    column![
        row![
            column![text("GPU").size(20), text("Current").size(10),].spacing(2),
            Space::with_width(Length::Fill),
            column![text(format_frequency(state.current_gpu_freq)).size(28),]
        ]
        .align_y(alignment::Vertical::Center)
        .spacing(2),
        Space::with_height(8),
...
fn create_power_view(state: &State) -> Element<Message> {
    column![
        row![
            column![text("Power").size(20), text("APU Total").size(10),].spacing(2),
            Space::with_width(Length::Fill),
            column![text(format!("{:.1} W", state.curr_stapm_value as f32 / 1000.0)).size(28),]
        ]
        .align_y(alignment::Vertical::Center)
        .spacing(2),
        Space::with_height(8),
